...
const likes = (names) => {
  const secondToLastName = names[names.length - 2];
  const lastName = names[names.length - 1];
  const lastTwoNamesString = `${secondToLastName} and ${lastName} like this`;
  let extraNamesString = "";
  
  if (names.length === 0) { return `no one likes this`; }
  if (names.length === 1) { return `${names[0]} likes this`; }
  if (names.length === 2) { return lastTwoNamesString; }
